...
def parse_train_args():
    ...
    parser.add_argument("--flow_method", choices=['original', 'cdf_trick', 'unsid', 'tuned'], default='cdf_trick')
    ...
    parser.add_argument("--alpha_scale", type=float, default=4, help='controls the alphas at which training is performed by scaling them. If this is higher, then higher alphas are sampled for training. Recall that alphas correspond to diffusion time in dirichlet flow matching. alpha=1 corresponds to full noise, and alpha_max, such as alpha_max = 8 corresponds to clean data.')
    parser.add_argument("--alpha_max", type=float, default=16, help='controls the maximum value until which we run the inference process. In equation 14, we write our probability path to go from alpha=1 to alpha=infinity. In practice we cut off alpha at alpha_max.')
